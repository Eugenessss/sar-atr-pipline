...
`src/` 공통 코드나 여러 파일이 필요할 때 사용. **Private 저장소라 토큰 필요.**
...
코랩 메뉴 → `파일 → GitHub에 사본 저장` → 저장소·브랜치 선택 → **원래 경로 그대로** 지정(예: `pipeline/soc14/dom48_convnext.ipynb`) → 커밋 메시지 입력. 출력(셀 결과) 포함된 채로 업로드됨.
...
> ⚠ **노트북은 1인 1파일** 권장 (.ipynb는 충돌 해결이 까다로움). 데이터셋·가중치(`*.tar/*.pth/*.tif`)는 `.gitignore`가 차단하지만, `git add .` 대신 **올릴 파일만 지정**하는 습관이 안전.
