...
읽는 순서: **detection → classification → pipeline**. baseline·diagnosis는 보조 분석입니다.

---

## 데이터셋 (구글 드라이브)

데이터·가중치는 용량이 커서 깃허브에 올리지 않습니다. 아래 구조로 **본인 드라이브에 준비**하고, 코랩에서 `MyDrive/ATRNet-STAR/` 경로로 마운트해 사용합니다.
...
│   └── *_ckpt.pth                    이어학습용 체크포인트 (옵티마이저 포함)
│
...
위 구조대로 `MyDrive/ATRNet-STAR/`에 `soc50.tar` · `coco_annotations.tar` · `dom_scenes.tar`를 배치합니다.
...
### 2. (선택) 저장소 전체를 코랩으로 클론

`src/` 공통 코드나 여러 파일이 필요할 때. **Private 저장소라 토큰이 필요**합니다.

```python
from getpass import getpass
token = getpass("GitHub 토큰: ")        # 노트북에 직접 적지 말 것 (입력창으로만)
!git clone https://{token}@github.com/<USER>/sar-atr-pipline.git
%cd sar-atr-pipline
```

> 토큰 발급: GitHub → Settings → Developer settings → Personal access tokens → **Fine-grained token** (해당 repo `Contents: Read and write` 권한)
...
런타임 → **T4 GPU** (SARATR-X는 A100 권장). **순서대로 실행**: `detection/stage1` → `classification/*` → `pipeline/*`. 각 노트북이 결과를 드라이브 `checkpoints/`·`results/`에 저장하고, 다음 노트북이 이를 읽어 이어집니다. 세션이 끊겨도 재실행하면 체크포인트부터 이어 학습합니다.

### 4. 실행 결과를 저장소에 푸시 (둘 중 택1)

**(A) 코랩 내장 기능 — 노트북 1개 올리기 (가장 쉬움)**

코랩 메뉴 → `파일 → GitHub에 사본 저장` → 저장소·브랜치 선택 → **원래 경로 그대로** 지정(예: `pipeline/soc14/dom48_convnext.ipynb`) → 커밋 메시지 입력. 출력(셀 결과)이 포함된 채로 올라갑니다.

**(B) git 명령어 — 여러 파일 한 번에**

```python
!git config user.email "you@example.com"
!git config user.name "유진"
!git add pipeline/ results_표.png        # 올릴 파일만 (데이터/가중치는 .gitignore가 차단)
!git commit -m "exp: soc14 dom48 결과 갱신"
!git push https://{token}@github.com/<USER>/sar-atr-pipline.git
```

> ⚠ **노트북은 1인 1파일**로 작업하세요(.ipynb는 충돌 해결이 까다로움). 데이터셋·가중치(`*.tar/*.pth/*.tif`)는 `.gitignore`가 막아주지만, `git add .` 대신 **올릴 파일만 지정**하는 습관이 안전합니다.
...
증강 레시피(RandomAffine + RandomErasing + label smoothing)로 세 모델 모두 논문 기준치를 넘었고, 탐지 단계 손실은 모델 무관 ~1.5%p로 일정합니다.
...
클래스를 줄일수록 정확도가 오릅니다(기종 식별 → 차종 대분류로 범위 축소하는 트레이드오프). recall이 동일한 건 탐지기가 같아서이며, 비교는 분류·E2E 기준으로 합니다.
...
DOM은 북쪽 고정 지오코딩이라 비0° 방위각 장면이 학습 칩과 좌표계가 어긋나 무너집니다. **장면을 방위각만큼 무손실 90° 회전(`np.rot90`) 후 추론, 박스만 원좌표로 역변환**하면 회복됩니다. 자세한 진단은 `diagnosis/dom_diagnosis.ipynb` 참고.
