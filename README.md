...
```
DOM 장면 → [1단계] YOLO(11n) + SAHI 타일 탐지 → 박스 중심 128px 크롭
...
```
sar-atr-pipline/
├── detection/                  1단계 — 탐지기 (버전 비교 후 YOLO11n 선정)
│   ├── stage1_yolo_detector.ipynb        YOLOv8n + SAHI (baseline)
│   ├── stage1_yolo11n.ipynb              YOLO11n ← 선정
│   └── stage1_yolo12n.ipynb              YOLO12n
│
...
│   └── soc14/
│       ├── dom48_convnext.ipynb              탐지기 v8n
│       └── dom48_convnext_yolo11n.ipynb      탐지기 11n (설정블록)
│
├── baseline/
...
│   ├── yolo_detector.pt              ← detection/stage1 (v8n)
│   ├── yolo_detector_yolo11n.pt      ← detection/stage1_yolo11n
│   ├── yolo_detector_yolo12n.pt      ← detection/stage1_yolo12n
...
### 5. 탐지기 버전 비교 (YOLO11n 선정)

| 탐지기 | 슬라이스 mAP50 | 슬라이스 Recall | 장면 recall (ID1) |
|---|---|---|---|
| YOLOv8n | 0.9517 | 0.8964 | 38/40 (95.0%) |
| **YOLO11n** | **0.9620** | **0.9211** | **39/40 (97.5%)** |
| YOLO12n | 0.9563 | 0.9032 | 34/40 (85.0%) |

장면 recall이 파이프라인에 직결되어 **11n 선정**. 12n은 슬라이스는 무난하나 장면에서 약하고 학습이 불안정(attention, 30에폭 부족). 탐지기만 11n으로 바꾸면 soc14 파이프라인 E2E 0.837 → **0.846**.

---

## 확정 설정값 (재현용)

- **탐지**: YOLO11n 선정(v8n/12n 비교), imgsz 128, 30에폭, 1클래스(vehicle) 통합. SAHI 타일 256 / overlap 0.25 / 100px 크기필터
...
- 탐지에 [Ultralytics YOLO](https://github.com/ultralytics/ultralytics)(v8n/11n/12n)와 [SAHI](https://github.com/obss/sahi)를 사용했습니다.

## License

- 본 저장소의 **코드**는 연구·학습 목적으로 작성되었습니다.
- **데이터셋·사전학습 가중치는 포함하지 않으며**, 각 원저작물의 라이선스·이용약관(ATRNet-STAR, SARATR-X)을 따릅니다
